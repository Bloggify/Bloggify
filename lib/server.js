...
const BloggifyServer = {
    ...
    _addCssUrl(url, filePath, save) {
        ...
        const resUrl = `${this.paths._paths.css_assets}${hash}-${basename}`;
     ...
     */
    handleBundleConfig(bloggifyConfig) {
        if (!bloggifyConfig) { return; }
        const bundles = bloggifyConfig.bundles || {};
     ...
     * @returns {object}         Returns the new Bundler or an existing one if its name is already used.
     */
    createBundler(options) {
        ...
        if (typeof options === "string") {
            options = {
                name: options,
                input: options.input || this.options.client_script,
                production: false,
        ...
        let paths = this.paths.getBundlePaths(options.name);
        options.bundle_dir = paths.bundle_dir;
        options.bundle_url = paths.bundle_url;
