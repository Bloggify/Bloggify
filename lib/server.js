...
module.exports = {
    _initServer () {
        ...
        let serveFirstRequests = () => {
            ...
            this.apis = {}
            if (isThere(this.paths.apis)) {
                this.apis = requireDir(this.paths.apis)
            }
            firstRequests._called = true;
            firstRequests.forEach(c => c());
            this.log(`The application is running at ${this.options.domain}`);
        ...
        this.server.beforeRequest.use((req, res, next) => {
            res.setHeader("X-Powered-By", "Bloggify");
            next();
        });
