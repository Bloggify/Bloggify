...
Cache.file = function (relPath, options, callback) {
    ...
    fileCache[relPath] = {
        buffer: [callback]
      , watch: Watch(Bloggify.server._sServer.root + relPath, function (err) {
        ...
        for (var i = 0; i < length; ++i) {
            ...
            if (Bloggify.config.blog.path === "/") {
                if (Bloggify.config.blog.slug) {
                    cData.url = "/" + Bloggify.config.blog.slug + "/";
                } else {
                    cData.url = "/";
                }
            } else {
                cData.url = Bloggify.config.blog.path + "/";
            }
            cData.url = cData.url + cData.id + "-" + cData.slug;
            readFile(cData);
        }
    }
