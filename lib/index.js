...
BloggifyLib.clearCache = function () {
    ...
    // Require config
    global.Config = Utils.requireNoCache(Config.root + "/config");
    BloggifyLib.initApi();
    ...
    Config.site.parsed.roots.pages["/login"] = {
        url: Config.site.paths.theme + "/core/html/login.html"
      , visible: false
      , slug: "login"
    };
    ...
    Config.site.parsed.roots.pages["/admin"] = {
        url: Config.site.paths.theme + "/core/html/admin/index.html"
      , label: "Admin"
      , order: 21
      , loggedIn: true
      , slug: "admin"
    ...
    Config.site.parsed.roots.pages["/admin/posts"] = {
        url: Config.site.paths.theme + "/core/html/admin/posts.html"
      , loggedIn: true
      , slug: "admin-posts"
      , wrap: false
    };
    ...
    Config.site.parsed.roots.pages["/admin/posts/new"] = {
        url: Config.site.paths.theme + "/core/html/admin/new-post.html"
      , loggedIn: true
      , slug: "new-post"
      , wrap: false
    };
...
BloggifyLib.initPlugins = function (callback) {
        ...
        (function (cPlugin, plugin) {
            ...
            Git.clone({
              ...
              , depth: 1
                ...
                // Set version/branch
                repository.exec("checkout", cPlugin.version, function (err, output) {
                    if (err) { return singleCallback(err); }

                    // Get package.json file
                    var packageJson = Utils.requireNoCache(pathToPlugin + "/package.json")
                    ...
                    Npm.load({prefix: pathToPlugin + "/node_modules"}, function (err) {
                        if (err) { return singleCallback(err); }
                        Npm.commands.install(packages, function (err, data) {
                            if (err && err.code !== "ENOPACKAGEJSON") { return singleCallback(err); }
                            Utils.requireNoCache(pathToPlugin);
...
BloggifyLib.start = function () {
    ...
    // Init plugins
    BloggifyLib.initPlugins(function (err, data) {
        if (err) {
